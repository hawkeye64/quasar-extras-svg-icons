  ...
  <section class="landing-page markdown-brand">
    <header class="hero">
      ...
      <div class="hero__grid">
        <div class="hero__copy">
          ...
          <p class="hero-lede">
            Ship icon-heavy Quasar apps with flattened SVG sets, predictable import paths, and a
            built-in icon finder instead of hand-curating every upstream package yourself.
          </p>
          ...
          <div class="hero-buttons">
            ...
            >
              <div class="hero-button__content q-anchor--skip">
                <span class="hero-button__slot hero-button__slot--empty" aria-hidden="true" />
                <span class="hero-button__label">Icon Finder</span>
                <span class="hero-button__slot">
    ...
    <section class="path-section">
      <div class="section-heading">
        <div class="section-heading__eyebrow">Start Here</div>
      ...
      <div class="path-grid">
        <router-link v-for="card in pathCards" :key="card.title" :to="card.path" class="path-card">
          ...
          <div class="path-card__eyebrow">{{ card.eyebrow }}</div>
          <h3 class="path-card__title">{{ card.title }}</h3>
          <p class="path-card__body">{{ card.body }}</p>
    ...
    <section class="showcase-section">
      <div class="section-heading">
        <div class="section-heading__eyebrow">Featured</div>
        <h2 class="section-heading__title">A few icon families you can reach for right away</h2>
        <p class="section-heading__text">
          The package spans product icons, brands, pictograms, emojis, flags, and more. Here are a
          few families that give a quick feel for the range.
        </p>
      </div>
      ...
      <div class="showcase-grid">
        ...
        >
          ...
          />
          <div class="showcase-card__meta">{{ card.meta }}</div>
          <h3 class="showcase-card__title">{{ card.title }}</h3>
          <p class="showcase-card__body">{{ card.body }}</p>
        </router-link>
      </div>
    ...
    <section class="resource-section">
      <article class="resource-card resource-card--primary">
        ...
        <div class="resource-card__actions">
          ...
          <a
            href="https://www.npmjs.com/package/quasar-extras-svg-icons"
            target="_blank"
            rel="noopener noreferrer"
            class="resource-link"
          >
...
.landing-page {
  --landing-border: rgba(230, 245, 255, 0.12);
  --landing-border-strong: rgba(230, 245, 255, 0.2);
  --landing-surface: rgba(11, 24, 41, 0.68);
  --landing-surface-strong: rgba(17, 36, 61, 0.84);
  --landing-text-soft: rgba(236, 247, 255, 0.82);
  --landing-shadow: 0 26px 56px rgba(4, 10, 20, 0.28);
  position: relative;
...
.landing-page::before {
  ...
  inset: 0;
  background:
    radial-gradient(circle at 12% 12%, rgba(38, 166, 154, 0.18), transparent 26%),
    radial-gradient(circle at 82% 8%, rgba(25, 118, 210, 0.2), transparent 32%),
    linear-gradient(180deg, rgba(8, 18, 30, 0.96), rgba(6, 14, 25, 0.98));
  pointer-events: none;
}
...
.hero__mesh--left {
  ...
  width: 240px;
  height: 240px;
  background: radial-gradient(circle at center, rgba(38, 166, 154, 0.26), transparent 68%);
}
...
.hero__mesh--right {
  ...
  width: 280px;
  height: 280px;
  background: radial-gradient(circle at center, rgba(25, 118, 210, 0.28), transparent 72%);
}
...
.hero-button--solid {
  background: #f1fbff;
  color: #0b2540;
  box-shadow: 0 18px 30px rgba(5, 12, 22, 0.2);
}
...
.hero-pill {
  padding: 8px 12px;
  border: 1px solid rgba(241, 251, 255, 0.12);
  border-radius: 999px;
  background: rgba(241, 251, 255, 0.05);
  color: #b9f6ee;
  font-size: 0.84rem;
  font-weight: 600;
}
...
.resource-card {
  ...
  box-shadow: 0 16px 34px rgba(4, 10, 20, 0.16);
}
...
.preview-stat {
  padding: 16px;
  border: 1px solid rgba(241, 251, 255, 0.1);
  border-radius: 18px;
  background: rgba(241, 251, 255, 0.05);
}
...
.showcase-card::before {
  ...
  width: 100%;
  height: 1px;
  background: linear-gradient(90deg, rgba(38, 166, 154, 0.58), transparent 68%);
}
...
.showcase-card {
  background:
    linear-gradient(180deg, rgba(236, 247, 255, 0.05), rgba(236, 247, 255, 0.02)),
    var(--landing-surface);
}

.showcase-card__image {
  margin-bottom: 16px;
  border: 1px solid rgba(241, 251, 255, 0.08);
  border-radius: 18px;
...
.resource-card--primary {
  background:
    radial-gradient(circle at top right, rgba(38, 166, 154, 0.18), transparent 30%),
...
.landing-page {
  ...
  --landing-shadow: 0 28px 60px #{rgba($brand-primary, 0.12)};
  --landing-card-shadow: 0 16px 34px #{rgba($brand-secondary, 0.12)};
  --landing-chip-bg: #{rgba($brand-light, 0.54)};
  ...
  --landing-panel-gradient-top: #{rgba($brand-light, 0.72)};
  --landing-panel-gradient-bottom: #{rgba($brand-light, 0.2)};
  --landing-panel-bg: #{rgba($brand-light-bg, 0.9)};
  --landing-preview-card-border: #{rgba($brand-secondary, 0.16)};
  --landing-preview-card-bg: #{rgba($brand-light, 0.82)};
  --landing-stat-border: #{rgba($brand-secondary, 0.12)};
  --landing-stat-bg: #{rgba($brand-light, 0.48)};
  --landing-accent-line: #{rgba($brand-primary, 0.42)};
  --landing-icon-bg: #{rgba($brand-primary, 0.1)};
  --landing-icon-color: #{$brand-primary};
  --landing-meta-text: #{$brand-primary};
  --landing-showcase-image-border: #{rgba($brand-secondary, 0.1)};
  --landing-showcase-image-bg: #{rgba($brand-light, 0.72)};
  --landing-spot-accent: #{rgba($brand-primary, 0.14)};
  --landing-resource-link-bg: #{rgba($brand-light, 0.5)};
  ...
  --landing-page-overlay-1: #{rgba($brand-primary, 0.1)};
  --landing-page-overlay-2: #{rgba($brand-secondary, 0.08)};
  --landing-page-overlay-start: #{rgba($brand-light, 0.96)};
  --landing-page-overlay-end: #{rgba($brand-light-bg, 0.98)};
...
body.body--dark .landing-page {
  ...
  --landing-shadow: 0 28px 60px #{rgba($brand-dark-bg, 0.3)};
  ...
  --landing-panel-bg: #{rgba($brand-dark-bg, 0.58)};
  --landing-preview-card-border: #{rgba($brand-light, 0.12)};
  --landing-preview-card-bg: #{rgba($brand-dark-bg, 0.78)};
  --landing-stat-border: #{rgba($brand-light, 0.08)};
  --landing-stat-bg: #{rgba($brand-light, 0.05)};
  --landing-accent-line: #{rgba($brand-primary, 0.55)};
  --landing-icon-bg: #{rgba($brand-primary, 0.16)};
  --landing-icon-color: #{$brand-primary};
  --landing-meta-text: #{$brand-primary};
  --landing-showcase-image-border: #{rgba($brand-light, 0.08)};
  --landing-showcase-image-bg: #{rgba($brand-dark-bg, 0.74)};
  ...
  --landing-accent-text: #{$brand-primary};
  --landing-page-overlay-1: #{rgba($brand-primary, 0.16)};
  --landing-page-overlay-2: #{rgba($brand-light, 0.08)};
  --landing-page-overlay-start: #{rgba($brand-dark-bg, 0.96)};
  --landing-page-overlay-end: #{rgba($dark-pill, 0.98)};
...
.landing-page::before {
  background:
    radial-gradient(circle at 12% 12%, var(--landing-page-overlay-1), transparent 26%),
    radial-gradient(circle at 82% 8%, var(--landing-page-overlay-2), transparent 32%),
    linear-gradient(180deg, var(--landing-page-overlay-start), var(--landing-page-overlay-end));
}
...
.preview-stat {
  border-color: var(--landing-stat-border);
...
.feature-card::before,
.showcase-card::before {
  background: linear-gradient(90deg, var(--landing-accent-line), transparent 68%);
}
...
.showcase-card__image {
  border-color: var(--landing-showcase-image-border);
  background: var(--landing-showcase-image-bg);
}
